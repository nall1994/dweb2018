$(() => {
    $('#addToFavorites').click(e => {
        if($('#addToFavorites').text()== "Adicionar aos favoritos"){
            e.preventDefault()
            ...
            $('#addToFavorites').text("Remover dos favoritos");
        }
        else{
            e.preventDefault()
            var dados = {favoriteEmail: $('#favoriteEmail').val(), favoriteNome: $('#favoriteNome').val()}
            $.ajax({
                type: "POST",
                url: "http://localhost:3000/users/remFromFavorites",
                data: JSON.stringify(dados), 
                contentType: "application/json; charset=utf-8",
            //  dataType: "json",
                success: msg => {
            ...
            $('#addToFavorites').text("Adicionar aos favoritos");
        }
    })
