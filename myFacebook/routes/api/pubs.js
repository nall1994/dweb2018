...
// Falta pôr em uso e possivelmente corrigir.
router.get('/fromGroup', passport.authenticate('jwt', { session: false }), (req, res) => {
    var seletores = new Object();
    seletores.groupId = req.query.group_id;
    // seletores.groupId = { $exists: true };
    pubsController.consulta(seletores)
        .then(dados => res.jsonp(dados))
        .catch(erro => res.jsonp({ message: 'Erro: ' + erro }))
});
